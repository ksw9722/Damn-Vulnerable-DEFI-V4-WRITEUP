...
contract CurvyPuppetLending is ReentrancyGuard {
    ...
    constructor(address _collateralAsset, IStableSwap _curvePool, IPermit2 _permit2, CurvyPuppetOracle _oracle) {
        ...
        collateralAsset = _collateralAsset; // collateralAsset = DVT
    ...
    function getCollateralValue(uint256 amount) public view returns (uint256) {
        if (amount == 0) return 0;
        return amount.mulWadDown(oracle.getPrice(collateralAsset).value); //collateralAsset = DVT
    }
    ...
    function _getLPTokenPrice() private view returns (uint256) {
        return oracle.getPrice(curvePool.coins(0)).value.mulWadDown(curvePool.get_virtual_price()); // coins(0)=ETH
    }
}
